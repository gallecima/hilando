        ...
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
            <div class="flex-grow-1">
                ...
                <h2 class="fs-base lh-base fw-medium text-muted mb-0">
                    Crear nuevo método de envío para el checkout
                </h2>
            </div>
...
<div class="content">
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">Nuevo Método de Envío</h3>
        </div>
        <div class="block-content block-content-full">
            <div class="row">
                <div class="col-lg-4">
                    <p class="fs-sm text-muted">
                        Definí un nuevo método de envío incluyendo valores de costo, zona, tiempo estimado, descuentos y dimensiones.
                    </p>
                    @if ($errors->any())
                ...
                <div class="col-lg-8 space-y-4">
                    <form action="{{ route('admin.shipmentmethod.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">$</span>
                                    <input type="number" step="0.01" class="form-control" id="min_cart_amount" name="min_cart_amount" value="{{ old('min_cart_amount') }}" placeholder="Ej: 10000.00">
                                </div>
                            </div>       
                        </div>                 
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="delay" name="delay" value="{{ old('delay') }}" required placeholder="Ej: 2 a 5 días hábiles">
                        </div>

                        <div class="mb-3">
                            <label for="discount_type" class="form-label">Tipo de Descuento</label>
                            <select class="form-select" id="discount_type" name="discount_type">
                                <option value="" {{ old('discount_type') === null ? 'selected' : '' }}>Sin descuento</option>
                                <option value="percent" {{ old('discount_type') === 'percent' ? 'selected' : '' }}>Porcentaje (%)</option>
                                <option value="amount" {{ old('discount_type') === 'amount' ? 'selected' : '' }}>Importe Fijo ($)</option>
                            </select>
                        </div>

                        <div class="mb-3">
                            <label for="discount_value" class="form-label">Valor del Descuento</label>
                            <input type="number" step="0.01" class="form-control" id="discount_value" name="discount_value" value="{{ old('discount_value') }}" placeholder="Opcional">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="shipping_point_id" name="shipping_point_id">
                                ...
                                @foreach($shippingPoints as $point)
                                    <option value="{{ $point->id }}" {{ (int) old('shipping_point_id') === (int) $point->id ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <div class="form-check form-switch mt-4">
                                    <input class="form-check-input" type="checkbox" name="allow_nearby_match" id="allow_nearby_match" value="1" {{ old('allow_nearby_match') ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control" id="nearby_radius_km" name="nearby_radius_km" value="{{ old('nearby_radius_km', '10') }}">
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" id="postal_code" name="postal_code" value="{{ old('postal_code') }}" placeholder="Ej: 1425">
                        ...
                        <div class="row">
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control" id="weight_limit" name="weight_limit" value="{{ old('weight_limit') }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="width_limit" class="form-label">Ancho (cm)</label>
                                <input type="number" class="form-control" id="width_limit" name="width_limit" value="{{ old('width_limit') }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="height_limit" class="form-label">Alto (cm)</label>
                                <input type="number" class="form-control" id="height_limit" name="height_limit" value="{{ old('height_limit') }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label for="length_limit" class="form-label">Largo (cm)</label>
                                <input type="number" class="form-control" id="length_limit" name="length_limit" value="{{ old('length_limit') }}">
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="shipping_boxes" name="shipping_boxes[]" multiple size="6">
                                @foreach($shippingBoxes as $box)
                                    <option value="{{ $box->id }}" {{ in_array($box->id, old('shipping_boxes', []), true) ? 'selected' : '' }}>
                            ...
                            <small class="text-muted">Si no seleccionás cajas, el método no restringe empaquetado. Si seleccionás, el checkout validará que el carrito entre en una combinación posible.</small>
                        </div>
