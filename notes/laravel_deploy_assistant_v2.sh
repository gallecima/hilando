...
ask_yes_no() {
  ...
  value="${value:-$default}"
  case "$value" in
    y|Y|yes|YES|s|S|si|SI) printf -v "$varname" '%s' "y" ;;
...
say "Asistente interactivo para deploy Laravel v2"
warn "Este script puede borrar la carpeta del proyecto y crear un nuevo server block en Nginx para el dominio indicado."
ask_yes_no "¿Querés continuar?" CONTINUE "n"
[[ "$CONTINUE" == "y" ]] || exit 0

DEFAULT_PROJECT_PATH="/var/www/html/proyecto"
...
ask "Dominio principal para crear el server block" DOMAIN
...
ask_yes_no "¿Crear un nuevo server block de Nginx para este dominio?" SETUP_NGINX "y"
ask "Tamaño máximo de subida para Nginx (client_max_body_size)" CLIENT_MAX_BODY_SIZE "$DEFAULT_CLIENT_MAX_BODY_SIZE"
ask_yes_no "¿Querés solicitar/instalar SSL con Certbot para este dominio?" RUN_CERTBOT "y"
...
if [[ "$SETUP_DB" == "y" ]]; then
  ...
  ask "Nombre de la base de datos" DB_NAME "proyecto_db"
  ask "Usuario MySQL local (Laravel en el VPS)" DB_USER_LOCAL "proyecto_user"
...
say "Verificando comandos básicos"
...
if [[ -f .env ]]; then
  ...
  warn "No encontré .env ni .env.example."
fi
...
if [[ -f artisan ]]; then
  ...
  php artisan key:generate --force || warn "No pude generar APP_KEY automáticamente."
...
if [[ "$SETUP_NGINX" == "y" ]]; then
  say "Creando server block nuevo para $DOMAIN"
  write_nginx_http_conf
  ln -sf "$NGINX_CONF_PATH" "/etc/nginx/sites-enabled/${DOMAIN}.conf"
  nginx_test_reload
  ok "Server block creado y habilitado"
fi
...
if [[ "$RUN_CERTBOT" == "y" ]]; then
  if ! command -v certbot >/dev/null 2>&1; then
    ...
    say "Solicitando SSL con Certbot para $DOMAIN ${WWW_DOMAIN:+y $WWW_DOMAIN}"
...
Este script:
- crea el proyecto en $PROJECT_PATH
- crea un server block NUEVO en $NGINX_CONF_PATH (si elegiste Nginx)
- pregunta si querés SSL y ejecuta Certbot (si elegiste SSL)

Pasos manuales pendientes:
1. Subir uploads desde tu máquina local con rsync, por ejemplo:
